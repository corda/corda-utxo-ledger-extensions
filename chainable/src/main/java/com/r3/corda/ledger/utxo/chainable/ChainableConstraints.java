...
public final class ChainableConstraints {
     ...
     * @param <T>     The underlying type of {@link ChainableState} to map.
    ...
    private static <T extends ChainableState<?>> Map<StaticPointer<T>, List<StaticPointer<T>>> mapInputsToOutputs(
        ...
        for (StateAndRef<T> input : inputs) {
            final StaticPointer<T> inputPointer = (StaticPointer<T>) new StaticPointer<>(
                    input.getRef(),
        ...
        for (ChainableState output : outputs) {
            final StaticPointer<T> outputPointer = output.getPreviousStatePointer();
